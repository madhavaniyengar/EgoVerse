...
class MimicgenFranka(Embodiment):
    ...
    @staticmethod
    def get_transform_list(
        mode: Literal["cartesian", "relative_cartesian"] = "cartesian",
    ) -> list[Transform]:
        if mode == "cartesian":
            return _build_franka_right_arm_cartesian_transform_list()
        if mode == "relative_cartesian":
            return _build_franka_right_arm_relative_transform_list()
        raise ValueError(f"Unsupported transform mode '{mode}'")
...
def _build_franka_right_arm_relative_transform_list(
    *,
    right_cmd_world: str = "right.cmd_ee_pose",
    right_obs_world: str = "right.obs_ee_pose",
    right_cmd_gripper: str = "right.cmd_gripper",
    ...
    """Express every future EEF target relative to the current EEF pose.

    The pose action at horizon step ``i`` is ``inv(T_obs) @ T_cmd[i]``.  The
    gripper remains an absolute normalized command, matching the existing
    Franka action head's 7D ``xyz + ypr + gripper`` layout.
    """
    return [
        ActionChunkCoordinateFrameTransform(
