...
Zarr data stored at:
- `/home/ubuntu/data/mimicgen_pickplace_franka_zarr/`
- `/home/ubuntu/data/mimicgen_pickplace_sawyer_zarr/`
...
The default co-training batch is 512 samples per domain on the GH200 (512 Franka
+ 512 Sawyer). The Franka-only baseline defaults to 128 samples per step; override
its batch size when equal per-step Franka exposure is required for a comparison.

## Training

Run from the repository root with the `emimic` environment active. On a
single-GPU machine, train the methods sequentially:

```bash
python egomimic/trainHydra.py --config-name=train_zarr_mimicgen_cotrain
python egomimic/trainHydra.py --config-name=train_zarr_mimicgen_franka_only
```

If normalization statistics have already been computed, both runs can reuse the
same paired-data cache (the file contains entries for both embodiments):

```bash
NORM_STATS=/path/to/norm_stats/norm_stats.json

python egomimic/trainHydra.py \
  --config-name=train_zarr_mimicgen_cotrain \
  norm_stats.precomputed_norm_path="$NORM_STATS" \
  norm_stats.save_cache_dir=null

python egomimic/trainHydra.py \
  --config-name=train_zarr_mimicgen_franka_only \
  norm_stats.precomputed_norm_path="$NORM_STATS" \
  norm_stats.save_cache_dir=null
```
